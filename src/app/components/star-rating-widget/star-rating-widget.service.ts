...
export class StarRatingWidgetService {
  ...
  httpSendRate(rate, message): Observable<string> {
    ...
    if (rate && token && projectId && recipientId) {
      ...
      // const url = this.API_URL + 'chat/support/tilechat/requests/' + recipientId + '/rate?token=[REDACTED_TOKEN],&rating=' 
      // tslint:disable-next-line:max-line-length
      const url = this.API_URL + projectId + '/requests/' + recipientId + '/rating';
      // 'chat/support/tilechat/requests/' + recipientId + '/rate?token=[REDACTED_TOKEN],&rating=' + rate + '&rating_message=' + message;
